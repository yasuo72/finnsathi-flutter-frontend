...
class AIChatService extends ChangeNotifier {
  ...
  Future<void> _loadApiKey() async {
    try {
      ...
      debugPrint('API Key loaded: ${_apiKey != null && _apiKey!.isNotEmpty ? "Found (starts with ${_apiKey!.substring(0, 5)}...)" : "Not found"}');
      ...
      try {
        ...
        _messages = decodedList.map((item) {
          return ChatMessage(
            id: item['id'],
            text: item['text'],
            sender: item['sender'] == 'user' ? MessageSender.user : MessageSender.ai,
  ...
  Future<void> sendMessage(String text) async {
    ...
    debugPrint('Sending message: "${text.substring(0, text.length > 20 ? 20 : text.length)}..."');
    ...
    try {
      ...
      try {
        ...
        response = await _generateResponse(text)
      ...
      final responseIndex = _messages.indexWhere((m) => m.id == loadingMessage.id);
      if (responseIndex != -1) {
  ...
  Future<String> _callOpenAI(String prompt) async {
    debugPrint('Starting API call for prompt: "${prompt.substring(0, prompt.length > 20 ? 20 : prompt.length)}..."');
    ...
    try {
      ...
      final systemPrompt = 'You are a helpful financial assistant. You provide personalized financial advice based on the user\'s data. ' +
                    'Always be respectful and professional. Keep responses concise and helpful.';
                    
      final userMessage = 'Here is my financial information:\n$financialContext\n\nMy question is: $prompt';
      ...
      debugPrint('Request body: ${requestBody.substring(0, requestBody.length > 100 ? 100 : requestBody.length)}...');
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('Response content: ${content.substring(0, content.length > 50 ? 50 : content.length)}...');
        return content;
      } else {
  ...
  Future<Map<String, double>> _getMonthlyStats(int year, int month) async {
    ...
    final transactions = _financeService.getTransactionsForDateRange(startOfMonth, endOfMonth);
  ...
  Future<String> _getFinancialContext() async {
    ...
    // Get financial data
    final currentMonthData = await _getMonthlyStats(thisMonth.year, thisMonth.month);
    final lastMonthData = await _getMonthlyStats(lastMonth.year, lastMonth.month);
  ...
  Future<String> _getFallbackResponse(String query) async {
    ...
    if (lowercaseQuery.contains('spend') || lowercaseQuery.contains('expense')) {
      return await _getExpenseAnalysis(query);
    } else if (lowercaseQuery.contains('income') || lowercaseQuery.contains('earn')) {
      return await _getIncomeAnalysis(query);
    } else if (lowercaseQuery.contains('save') || lowercaseQuery.contains('saving')) {
      return await _getSavingsAnalysis(query);
    } else if (lowercaseQuery.contains('compare') || lowercaseQuery.contains('difference')) {
      return await _getComparisonAnalysis(query);
    } else if (lowercaseQuery.contains('budget') || lowercaseQuery.contains('plan')) {
  ...
  Future<String> _getExpenseAnalysis(String query) async {
    ...
    final categorySpending = _financeService.getCategorySpending(startOfMonth, endOfMonth);
    
    // Find top spending categories
    final expenseCategories = categorySpending.entries
      .where((e) => TransactionCategory.values[e.key.index].transactionType == TransactionType.expense)
      .toList()
    ...
    if (expenseCategories.isNotEmpty) {
      ...
      for (int i = 0; i < expenseCategories.length && i < 3; i++) {
        ...
        topCategoriesText += '• ${category.displayName}: \$${amount.toStringAsFixed(2)} (${percentage.toStringAsFixed(1)}%)\n';
      }
    }
  ...
  Future<String> _getIncomeAnalysis(String query) async {
    ...
    // Get category breakdown
    final categoryIncome = _financeService.getCategorySpending(startOfMonth, endOfMonth);
    
    // Find income categories
    final incomeCategories = categoryIncome.entries
      .where((e) => TransactionCategory.values[e.key.index].transactionType == TransactionType.income)
      .toList()
      ..sort((a, b) => b.value.compareTo(a.value));
    ...
    if (incomeCategories.isNotEmpty) {
      ...
      for (int i = 0; i < incomeCategories.length; i++) {
        ...
        incomeCategoriesText += '• ${category.displayName}: \$${amount.toStringAsFixed(2)} (${percentage.toStringAsFixed(1)}%)\n';
      }
    }
    ...
    final prevMonthTransactions = _financeService.getTransactionsForDateRange(
      startOfPrevMonth, 
      endOfPrevMonth
    ).where((t) => t.type == TransactionType.income).toList();
    
    final prevMonthIncome = prevMonthTransactions.fold(0.0, (sum, t) => sum + t.amount);
    ...
    if (prevMonthIncome > 0) {
      ...
      if (difference > 0) {
        comparisonText = "That's an increase of \$${difference.toStringAsFixed(2)} (${percentChange.toStringAsFixed(1)}%) compared to last month.";
      } else if (difference < 0) {
        comparisonText = "That's a decrease of \$${(-difference).toStringAsFixed(2)} (${(-percentChange).toStringAsFixed(1)}%) compared to last month.";
      } else {
  ...
  Future<String> _getSavingsAnalysis(String query) async {
    ...
    for (final goal in savingsGoals) {
      ...
      if (goal.targetDate != null) {
        ...
        if (daysRemaining > 0) {
          timeRemainingText = " with $daysRemaining days remaining to reach your target date";
        } else if (daysRemaining == 0) {
          timeRemainingText = " (your target date is today)";
        } else {
          timeRemainingText = " (you've passed your target date by ${-daysRemaining} days)";
        }
      }
      
      goalsText += "**${goal.title}**\n"
    ...
    if (savingsRate < 10) {
      savingsAdvice = "Your current savings rate is ${savingsRate.toStringAsFixed(1)}%, which is below the recommended 20%. Consider reviewing your expenses to increase your savings rate.";
    } else if (savingsRate < 20) {
      savingsAdvice = "Your current savings rate is ${savingsRate.toStringAsFixed(1)}%. That's a good start, but aiming for 20% or more would help you reach your goals faster.";
    } else {
      savingsAdvice = "Great job! Your current savings rate is ${savingsRate.toStringAsFixed(1)}%, which exceeds the recommended 20%. You're on track to reach your goals.";
    }
  ...
  Future<String> _getComparisonAnalysis(String query) async {
    ...
    String result = "**Income vs Expenses (This Month)**\n\n"
    ...
    final prevSavingsRate = prevIncome > 0 ? (prevBalance / prevIncome) * 100 : 0.0;
    
    if (prevIncome > 0 || prevExpenses > 0) {
      result += "**Comparison with Last Month**\n\n"
    ...
    if (balance < 0) {
      result += "⚠️ Your expenses exceed your income this month. Consider cutting back on non-essential spending to bring your budget back into balance.";
    } else if (savingsRate < 10) {
      result += "You're saving ${savingsRate.toStringAsFixed(1)}% of your income, which is below the recommended 20%. Try to identify areas where you could reduce expenses to increase your savings rate.";
    } else if (savingsRate < 20) {
      result += "You're saving ${savingsRate.toStringAsFixed(1)}% of your income, which is decent but below the ideal 20%. With a few adjustments to your spending, you could reach that goal.";
    } else {
      result += "Great job! You're saving ${savingsRate.toStringAsFixed(1)}% of your income, which exceeds the recommended 20%. Keep up the good work!";
    }
  ...
  String _getChangeText(double current, double previous, {bool isPercentage = false}) {
    if (previous == 0) return isPercentage 
    ...
    String changeText = isPercentage
  ...
  Future<String> _getBudgetAnalysis(String query) async {
    ...
    for (final budget in budgets) {
      ...
      if (percentUsed >= 100) {
        ...
        statusText = "⚠️ Approaching limit (${percentUsed.toStringAsFixed(1)}%)";
      } else {
    ...
    final approachingLimitCount = budgets.where((b) => b.spent <= b.limit && (b.spent / b.limit) >= 0.8).length;
    
    if (overBudgetCount > 0) {
      result += "You are over budget in $overBudgetCount ${overBudgetCount == 1 ? 'category' : 'categories'}. Consider adjusting your spending or increasing these budget limits if necessary.";
    } else if (approachingLimitCount > 0) {
      result += "You are approaching your budget limit in $approachingLimitCount ${approachingLimitCount == 1 ? 'category' : 'categories'}. Monitor these closely for the rest of the period.";
    } else {
  ...
  String _getFinancialAdvice(String query) {
    ...
    else if (lowercaseQuery.contains('spend') || lowercaseQuery.contains('expense')) {
