...
class Quest {
  ...
  bool get isCompleted => challenges.every((challenge) => challenge.isCompleted);
...
class GamificationService extends ChangeNotifier {
  ...
  Future<void> addBadge(String badgeId) async {
    if (!_userBadges.contains(badgeId)) {
      ...
      // Show notification
      _showNotification('New Badge Unlocked!', 'You earned the $badgeId badge!');
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getAllBadges() async {
    try {
      ...
      if (data != null && data['success'] == true && data['data'] != null && data['data']['badges'] != null) {
  ...
  Future<List<Map<String, dynamic>>> getLeaderboard() async {
    try {
      // Try to get from backend first
      final data = await ApiService.get('${ApiConfig.gamification}/leaderboard');
      
      if (data != null && data['success'] == true && data['data'] != null && data['data']['leaderboard'] != null) {
  ...
  Future<Map<String, dynamic>> refreshFinancialInsights() async {
    try {
      ...
      final data = await ApiService.get('${ApiConfig.gamification}/financial-health');
      
      if (data != null && data['success'] == true && data['data'] != null) {
        final score = data['data']['score'] ?? 50;
        final breakdown = data['data']['breakdown'] ?? {
  ...
  Map<String, dynamic> _calculateFinancialHealthScore() {
    try {
      ...
      if (budgets.isNotEmpty) {
        final adherenceRates = budgets.map((budget) {
          if (budget.limit <= 0) return 1.0;
          final adherence = 1 - (budget.spent / budget.limit).clamp(0.0, 1.0);
          return adherence;
        }).toList();
        
        final avgAdherence = adherenceRates.fold<double>(0, (sum, rate) => sum + rate) / adherenceRates.length;
      ...
      final score = (savingsRate + budgetAdherence + expenseManagement + goalProgress).round();
  ...
  Future<Map<String, dynamic>?> _fetchGamificationDataFromBackend() async {
    try {
      debugPrint('Fetching gamification data from backend: ${ApiConfig.gamification}');
  ...
  Future<void> _processBackendGamificationData(Map<String, dynamic> data) async {
    try {
      // Update streak data
      if (data['streak'] != null) {
      ...
      if (data['challenges'] != null && data['challenges'] is List) {
        _dailyChallenges = (data['challenges'] as List)
      ...
      if (data['achievements'] != null && data['achievements'] is List) {
        ...
        final achievementsList = (data['achievements'] as List)
            .map((a) => {
                  ...
                  'dateUnlocked': a['unlockedDate'] != null ? DateTime.parse(a['unlockedDate']) : null,
                })
            .toList();
        ...
        if (achievementsList.isNotEmpty) {
          try {
            ...
            for (final achievement in achievementsList) {
              if (achievement['isUnlocked'] == true && !_userBadges.contains(achievement['id'])) {
                _userBadges.add(achievement['id']);
              }
            }
            debugPrint('Updated ${achievementsList.length} achievements locally');
          } catch (e) {
  ...
  Future<void> _fetchChallengesFromBackend() async {
    try {
      debugPrint('Fetching challenges from backend: ${ApiConfig.gamification}/challenges');
      final data = await ApiService.get('${ApiConfig.gamification}/challenges');
      
      if (data != null && data['success'] == true && data['data'] != null && data['data']['challenges'] != null) {
        final List<dynamic> challengesJson = data['data']['challenges'];
        _dailyChallenges = challengesJson.map((json) => Challenge.fromJson(json)).toList();
        await _saveChallenges();
        debugPrint('Successfully fetched ${_dailyChallenges.length} challenges from backend');
      } else {
  ...
  Future<void> _loadGamificationData() async {
    try {
      ...
      if (isAuthenticated) {
        debugPrint('User is authenticated. Attempting to fetch gamification data from backend');
        final backendData = await _fetchGamificationDataFromBackend();
        ...
        if (backendData != null) {
          ...
          debugPrint('Failed to load from backend, falling back to local storage');
        }
      } else {
      ...
      _lastActivityDate = lastActivityStr != null 
  ...
  Future<void> _saveGamificationData() async {
    try {
      ...
      if (_financialInsights.isNotEmpty) {
        await prefs.setString('financial_insights', jsonEncode(_financialInsights));
      }
      
      // Save personalized tips
      if (_personalizedTips.isNotEmpty) {
        await prefs.setString('personalized_tips', jsonEncode(_personalizedTips));
      }
  ...
  Future<bool> _saveGamificationDataToBackend() async {
    try {
      debugPrint('Saving gamification data to backend: ${ApiConfig.gamification}');
      ...
      final Map<String, dynamic> gamificationData = {
        ...
        'badges': _userBadges,
        'coins': _financeCoins,
        'financialHealthScore': _financialInsights['score'],
        'challenges': _dailyChallenges.map((c) => c.toJson()).toList(),
      };
      
      final data = await ApiService.put(
        ApiConfig.gamification,
        gamificationData,
      );
      
      if (data != null && data['success'] == true) {
        debugPrint('Successfully saved gamification data to backend');
        return true;
      }
      
      debugPrint('Failed to save gamification data to backend: ${data?['message'] ?? 'Unknown error'}');
      return false;
    } catch (e) {
  ...
  void _awardStreakBonus() {
    ...
    if (_currentStreak % 7 == 0) {
      // Weekly streak bonus
      _financeCoins += 50;
      _showNotification('Weekly Streak Bonus!', 'You earned 50 Finance Coins for a 7-day streak!');
    } else if (_currentStreak % 30 == 0) {
      // Monthly streak bonus
      _financeCoins += 200;
      _showNotification('Monthly Streak Bonus!', 'You earned 200 Finance Coins for a 30-day streak!');
    } else {
  ...
  Future<bool> _recordActivityOnBackend() async {
    try {
      debugPrint('Recording activity on backend: ${ApiConfig.gamification}/record-activity');
      final data = await ApiService.post(
        '${ApiConfig.gamification}/record-activity',
        {}
      );
  ...
  Future<bool> completeChallenge(String challengeId) async {
    try {
      ...
      final challenge = _dailyChallenges.firstWhere(
        (c) => c.id == challengeId,
        orElse: () => _weeklyChallenges.firstWhere(
          (c) => c.id == challengeId,
          orElse: () => Challenge(
            id: challengeId,
      ...
      final backendSuccess = await _updateChallengeCompletionOnBackend(challengeId);
  ...
  Future<void> _checkAndGenerateDailyChallenges() async {
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      final lastChallengeGenDateStr = prefs.getString('last_challenge_generation_date');
      final lastChallengeGenDate = lastChallengeGenDateStr != null 
      ...
      if (lastChallengeGenDate == null || 
        ...
        await prefs.setString('last_challenge_generation_date', today.toIso8601String());
        await _saveChallenges();
      }
    } catch (e) {
  ...
  void _generateDailyChallenges() {
    ...
    final challengeTypes = ['transaction', 'budget', 'saving', 'streak', 'education'];
    final random = Random();
  ...
  Future<void> _loadChallenges() async {
    try {
      ...
      // First try to load from backend if user is authenticated
      if (isAuthenticated) {
        debugPrint('User is authenticated. Attempting to fetch challenges from backend');
        final backendChallenges = await _getChallengesFromBackend();
        
        if (backendChallenges != null && backendChallenges.isNotEmpty) {
          debugPrint('Successfully loaded ${backendChallenges.length} challenges from backend');
          _dailyChallenges = backendChallenges;
          return; // Successfully loaded from backend, no need to load from local storage
        } else {
          debugPrint('Failed to load challenges from backend or no challenges found, falling back to local storage');
        }
      } else {
        debugPrint('User not authenticated. Loading challenges from local storage only');
      }
      ...
      if (challengesJson != null) {
        ...
        _dailyChallenges = decoded
            .map((item) => Challenge.fromJson(item as Map<String, dynamic>))
            .toList();
        debugPrint('Loaded ${_dailyChallenges.length} challenges from local storage');
      } else {
  ...
  Future<void> _saveChallenges() async {
    try {
      ...
      // Always save locally as a backup
      final challengesJson = jsonEncode(_dailyChallenges.map((c) => c.toJson()).toList());
  ...
  Future<bool> _saveChallengesOnBackend() async {
    try {
      debugPrint('Saving challenges to backend: ${ApiConfig.gamification}/challenges');
      ...
      debugPrint('Failed to save challenges to backend: ${data?['message'] ?? 'Unknown error'}');
      return false;
    } catch (e) {
  ...
  Future<void> _checkAchievements() async {
    try {
      ...
      if (profile.completedTransactions >= 10) {
        achievementsUpdated = _checkAndUnlockAchievement(
          achievements, 
          'ach_001', 
          true
        ) || achievementsUpdated;
  ...
  Future<List<dynamic>?> _getAchievementsFromBackend() async {
    try {
      final data = await ApiService.get('${ApiConfig.gamification}/achievements');
  ...
  Future<List<Challenge>?> _getChallengesFromBackend() async {
    try {
      ...
      if (data != null && data['success'] == true && data['data'] != null && data['data']['challenges'] != null) {
        final List<dynamic> challengesJson = data['data']['challenges'];
        final challenges = challengesJson.map((json) => Challenge.fromJson(json)).toList();
        debugPrint('Successfully fetched ${challenges.length} challenges from backend');
        return challenges;
      }
  ...
  // Create a notification in the backend
  Future<void> _createNotificationInBackend(String title, String message) async {
    try {
